...
The dependencies are Supabase Auth/Storage, an existing trusted `app_auth_map` with UUID `user_id` and `role`, and the shared `app_job_pool` and `app_job_feed_status` tables. Verify the role mapping's RLS/policies prevent browser users from creating or changing mappings. No historical managed-client Copilot APIs are needed.

If both feed tables are absent, apply `patches/applydesk-self-service-feed-foundation.sql`, then `patches/applydesk-patch10-self-service.sql` and `patches/applydesk-patch11-resume-preference.sql`, using the trusted SQL Editor `postgres` role. The feed foundation creates only those two tables, their constraints/index, RLS, and server-role permissions. It intentionally aborts if either table already exists. For existing feed tables, compare their columns and constraints with the foundation and skip it when compatible; review a scoped fix for missing fields instead of replaying the old migrations.

Patch 10 is transactional and rerunnable. It creates empty self-service tables; it does not import clients, copy resumes, alter existing memberships, update legacy profiles, or assign recruiters. Do not replay historical patches 5–9 to install this feature. In particular, patch 6 contains historical email data-preparation updates. The original full migration path remains supported for installations that already have it.

Patch 11 must follow patch 10 before deploying the current frontend. If patch 10 is already live, run only patch 11. Patch 11 is transactional and rerunnable; do not reapply patch 10 afterwards, which would restore an old application RPC overload.
...
## Application resume choice

`application_resume_source` defaults to `applydesk` for every self-service member. Members can explicitly choose `custom` in My resumes or before opening a new application; the preference persists on their own member row. Preference changes and new application creation serialize under the membership lock. A stale browser sending a different source is rejected so it cannot quietly use the wrong document.

For ApplyDesk applications, the saved snapshot includes the reviewed/formatted or prepared job-specific profile and exports a PDF from that frozen content. A custom application saves only the unchanged original file's path/name and source; it never presents edited profile text or generated exports as the custom file. Its saved keyword-match score is null because reviewed profile text may differ from the unchanged original. Replacing the library resume archives the prior original. Existing applications retain their saved source and original path/content even after a preference, profile, or library change. Historical snapshots without a source key keep their original ApplyDesk export behavior.

ApplyDesk records the selected resume and opens the employer site; it does not automatically attach a file or submit on that external website. The handoff and Applications list offer the saved resume to download and upload there.
...
- `fn_ss_get_my_workspace()` returns `{ok,user_id,application_resume_source,resumes,activity,applications,feed_status}`. Resumes are active versions. Activity includes the job record. Applications include immutable job and resume snapshots.
...
- `fn_ss_set_resume_preference(p_source)` accepts `applydesk` or `custom` and updates only the active calling member.
- `fn_ss_set_job_activity(p_job_id,p_state)` accepts `saved`, `skipped`, or `none`.
- `fn_ss_log_application(p_job_id,p_resume_id,p_resume_text,p_latex_text,p_score,p_status='opened',p_notes='',p_parsed_profile=null,p_resume_source=null)` returns `{ok,id,already,url}`. Opening the employer page records `opened`, not a completed submission. Reopening returns the initial snapshot and preserves progress. The optional source must agree with the member preference. The eight-argument calling convention remains compatible.
...
These tests create synthetic records and roll them back. Run the commands from the repository root against a fresh disposable PostgreSQL database; never point a bootstrap at production. Each bootstrap also creates cluster-wide test roles, so use a fresh local PostgreSQL cluster for each path.

To verify the minimal feed foundation plus self-service path:

```sh
psql "$LOCAL_TEST_DATABASE_URL" -v ON_ERROR_STOP=1 -f supabase/tests/bootstrap-local-self-service.sql
psql "$LOCAL_TEST_DATABASE_URL" -v ON_ERROR_STOP=1 -f supabase/patches/applydesk-patch10-self-service.sql
psql "$LOCAL_TEST_DATABASE_URL" -v ON_ERROR_STOP=1 -f supabase/patches/applydesk-patch11-resume-preference.sql
psql "$LOCAL_TEST_DATABASE_URL" -v ON_ERROR_STOP=1 -v self_service_only=true -f supabase/tests/self-service.sql
psql "$LOCAL_TEST_DATABASE_URL" -v ON_ERROR_STOP=1 -f supabase/tests/self-service-resume-preferences.sql
```

The `self_service_only` flag replaces five historical-workspace assertions with checks that those managed-client tables/RPCs remain absent. The other member, administrator, suspension, storage, and lifecycle checks are identical. Omit the flag to keep the original assertions for the full historical setup:
...
Patch 11 verification additionally covers initial/repeat migration, default choice, explicit custom persistence, stale-choice rejection, original-file retention after replacement, immutable application source/content, cross-member isolation, suspended/legacy account rejection, and browser download behavior.
