...
The upgraded `copilot.html` provides current US job discovery across 57 connected employer boards, private resume versions, reviewed profiles, matching, exports and application tracking. All sponsorship statuses and all current posting dates are included by default; roles span software, data, healthcare, manufacturing, energy, retail, logistics, sales, marketing and finance. Coverage is limited to these connected employers, not the whole US market. See [verified employer sources](workers/JOB-SOURCES.md) for board links and dated verification counts. It has its own signup/login session and member data. Existing recruiter-managed clients stay in Mission Control. Administrators manage self-service members at `copilot-admin.html` using their existing staff session.
